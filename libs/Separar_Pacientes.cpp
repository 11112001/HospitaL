...
void imprimirEstruct(Paciente*& paciente)
{
    ...
    while(p->id != 999999)
    {
        cout << p -> id << p->tipoDeDocumento << p->nombres << p->apellidos << p->fechaNacimiento << p->telefono << p->email << p->tipoDeDocumento << p->entidadSalud << p->medicinaPrepagada << "\n";   
        ++p;
    }
...
void cargarPacientes(const string& archivo) 
{
    ...
    string linea;
    Paciente *paciente = new Paciente[101];
    ...
    while (getline(archi, linea)) 
    {
        ...
        //paciente[cont] ---> *(paciente + cont)

        //Almacenar los datos del paciente o procesarlos
        cont ++;
    }
   paciente[cont].id=999999; 
    //copychar(paciente[cont].tipoDeDocumento, "fin", 3);
